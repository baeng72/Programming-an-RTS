...
public:
	...
	struct FLARE
	{
		...
		D3DXCOLOR m_color;	//Flare m_color
		float place;		//position along the ray
		float scale;		//Size of the flare
		int sourceFlare;	//Which flare from the texture to use
	};
...
private:
	float m_mainAlpha;			//Overall alpha
	bool m_inScreen;				//Inside screen bounds or not
	int m_type;					//Flare type
	D3DXVECTOR3 m_position;		//World position of light source
	std::vector<FLARE> m_flares;	//Array of flares
};
