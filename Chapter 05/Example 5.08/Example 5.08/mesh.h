...
public:
	...
	MESH();
	MESH(const char fName[], IDirect3DDevice9* Dev);
	~MESH();
	HRESULT Load(const char fName[], IDirect3DDevice9* Dev);
	void Render();
...
public:
	...
	void RenderProgressive();
	void SetLOD(int numFaces);
	long GetNumProgressiveFaces() { return m_pMesh->m_pProgressiveMesh->GetNumFaces(); }
