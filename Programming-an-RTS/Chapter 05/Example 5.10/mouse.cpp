...
void MOUSE::Init(Renderer::RenderDevice* pdevice,  std::shared_ptr<Renderer::ShaderManager>& shaderManager,Core::Window* pwindow) {
	...
	if (Core::GetAPI() == Core::API::Vulkan) {
		_sphereShader = std::unique_ptr<Renderer::Shader>(Renderer::Shader::Create(pdevice, shaderManager->CreateShaderData("../../../../Resources/Chapter 05/Example 5.10/shaders/Vulkan/shape.glsl", false)));
	}
	else {
		_sphereShader = std::unique_ptr<Renderer::Shader>(Renderer::Shader::Create(pdevice, shaderManager->CreateShaderData("../../../../Resources/Chapter 05/Example 5.10/shaders/GL/shape.glsl", false)));
	}
}
