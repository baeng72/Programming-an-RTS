...
class MESH {
	...
	//std::unique_ptr<Mesh::Mesh> _progressiveMesh;
	std::unique_ptr<Mesh::ProgressiveMesh> _progressiveMesh;
	std::unique_ptr<Renderer::Texture> _texture;
	std::vector<float> _progressiveVertices;
	std::vector<uint32_t> _progressiveIndices;
	//float _vertexStride;
...
public:
	...
	void RenderProgressive(mat4& matVP, mat4& world, Renderer::DirectionalLight& light);
	void Release();
	void SetWireframe(bool wireframe) { _shader->SetWireframe(wireframe); }
	void SetNumProgressiveFaces(int numFaces);
	uint32_t GetProgressiveFaceCount()const { 
		return _progressiveMesh->GetIndexCount() / 3;
	}
};
...
public:
	...
	void RenderProgressive(glm::mat4& viewProj, Renderer::DirectionalLight& light);
	void SetLOD(int numFaces);
	long GetNumProgressiveFaces();
