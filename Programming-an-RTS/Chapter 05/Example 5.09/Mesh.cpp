...
void MESH::Render(glm::mat4& matViewProj, glm::mat4& matWorld, Renderer::DirectionalLight& light)
{
	...
	if (Core::GetAPI() == Core::API::Vulkan) {
		...
		auto texmap = _texture.get();
		_shader->SetTexture("texmap", &texmap, 1);
	}
...
{
	if (Core::GetAPI() == Core::API::Vulkan) {
		_shader.reset(Renderer::Shader::Create(_pdevice, _shaderManager->CreateShaderData("../../../../Resources/Chapter 05/Example 5.09/shaders/Vulkan/mesh.glsl")));
		int texid = 0;
		std::vector<Renderer::Texture*> textures = { _texture.get() };
		_shader->SetTexture(texid, textures.data(), 1);
	}
	else {
		_shader.reset(Renderer::Shader::Create(_pdevice, _shaderManager->CreateShaderData("../../../../Resources/Chapter 05/Example 5.09/shaders/GL/mesh.glsl")));
	}
}
