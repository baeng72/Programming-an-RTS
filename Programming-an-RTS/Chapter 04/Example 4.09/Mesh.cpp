...
void MESH::Render(glm::mat4& matViewProj, glm::mat4& matWorld, Renderer::DirectionalLight& light)
{
	...
	Renderer::FlatShaderPushConst pushConst{_xform *matWorld};
...
bool MESH::Load( Renderer::RenderDevice* pdevice, std::shared_ptr<Renderer::ShaderManager> shaderManager, const char* pfilename) {
	...
	_mesh = std::unique_ptr<Renderer::Mesh>(model->GetMesh(Renderer::MeshType::position_normal_uv, 0));
	_texture = std::unique_ptr<Renderer::Texture>(model->GetTexture(Renderer::TextureType::diffuse, 0));
...
{
	_shader.reset(Renderer::Shader::Create(_pdevice, _shaderManager->CreateShaderData("../../../../Resources/Chapter 04/Example 4.09/shaders/TexturedDirectional.glsl")));
	int texid = 0;
