...
void PATCH::Render(Renderer::MeshShader*pshader)
{
...
{
	_shader.reset(Renderer::MeshShader::Create(_pdevice, _shaderManager->GetShaderData(Renderer::ShaderManager::ShaderType::FlatDirectional,wireframe)));
}
...
void TERRAIN::Init(Renderer::RenderDevice* pdevice,std::shared_ptr<Renderer::ShaderManager> shaderManager, INTPOINT size_)
{
	...
	_size = size_;	
	_shader.reset(Renderer::MeshShader::Create(pdevice,_shaderManager->GetShaderData( Renderer::ShaderManager::ShaderType::FlatDirectional)));
...
void TERRAIN::Render(glm::mat4&viewProj,glm::mat4&model,Renderer::DirectionalLight&light)
{
	...
	_shader->SetUniformData(&ubo, sizeof(ubo));
