...
namespace Mesh {
	...
	public:
		...
		virtual void Advance(std::shared_ptr<Core::ThreadPool>& pool, float time, bool loop = true)=0;
		virtual void SetPose(float time, bool loop = true) = 0;
		virtual void SetPose(std::shared_ptr<Core::ThreadPool>& pool, float time, bool loop)=0;
