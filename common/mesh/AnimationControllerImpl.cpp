...
namespace Mesh {
	...
	{
		_boneCount = (int)_skeleton.boneHierarchy.size();
	...
	void AnimationControllerImpl::Advance(std::shared_ptr<Core::ThreadPool>& pool, float time, bool loop)
	{
		pool->QueueJob([this, time, loop] {Advance(time, loop); });
	}
	...
	void AnimationControllerImpl::SetPose(std::shared_ptr<Core::ThreadPool>& pool, float time, bool loop)
	{
		pool->QueueJob([this, time, loop] {SetPose(time, loop); });
	}
