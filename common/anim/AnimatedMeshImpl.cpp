...
namespace Animation {
	...
	AnimatedMeshImpl::AnimatedMeshImpl(Renderer::RenderDevice* pdevice, float* pvertices, uint32_t vertSize, uint32_t vertStride, uint32_t* pindices, uint32_t indSize, Mesh::Skeleton& skeleton, std::vector<Mesh::AnimationClip>& animations)
		:_pdevice(pdevice),_skeleton(skeleton),_animations(animations){
		_blendClip = -1;
		_blendPcnt = 0.f;
		_currClip = 0;
		_clipTime = 0.f;
	...
	void AnimatedMeshImpl::Render(Renderer::Shader* pshader)
	{
		pshader->SetUniformData("skeleton", _matrixPalette.data(),(uint32_t)( sizeof(mat4) * _matrixPalette.size()));
		pshader->Bind();
