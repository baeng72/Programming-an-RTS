...
namespace std {
	...
	struct VulkanShaderData {
		...
		//std::vector<uint32_t> _uniformsLocations;
		//std::vector<std::string> _storageNames;
		//std::vector<uint32_t> _storageLocations;
		//std::vector<std::string> _textureNames;
		//std::vector<uint32_t> _textureLocations; //will hold default texture 1x1 pixel texture
	};
	class VulkanShaderManager : public  Renderer::ShaderManager {
		...
		void CompileShaders();
		void CompileShader(const std::string&name,const std::unordered_map<VkShaderStageFlagBits, std::string>& shaderSources);
	...
	public:		
		...
		virtual void* GetShaderData(const char* shaderPath) override;
